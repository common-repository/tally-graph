...
*   You can make basic use of Tally Graph without any knowledge of the Google Chart API, but you can also use nearly any [Google Chart API parameters][gapi].
...
You type a tag directly into a post or page using WordPress [shortcode format][]. To put a tag in a theme template, use [template tag with querystring parameters][1]. Both formats take the same parameters listed below.
...
*   *cht* 
    ...
    This is a [Google Chart API][gapi] parameter, the chart type. `bvs` is a vertical bar chart.

*   Any other [Google Chart API parameters][gapi] are passed along, so you can go nuts with all the options. You'll probably use at least `chtt`, the chart title.
